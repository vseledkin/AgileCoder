...
class OpenAIModel(ModelBackend):
    ...
    def run(self, *args, **kwargs) -> Dict[str, Any]:
        ...
        if (
            self.model_type == ModelType.GPT_3_5_AZURE
            or self.model_type == ModelType.GPT_4_32k
        ):
        ...
        response = openai.chat.completions.create(
            *args, **kwargs, **self.model_config_dict
        )
...
class AuthropicClaudeAIModel(ModelBackend):
    ...
    def run(self, *args, **kwargs) -> Dict[str, Any]:
        ...
        # num_max_token = num_max_token_map[self.model_type.value]
        # num_max_completion_tokens = num_max_token - num_prompt_tokens
        self.model_config_dict["max_tokens"] = num_max_token_map.get(
            self.model_type.value, 4096
        )
        # self.model_config_dict
        # if self.model_type == ModelType.GPT_3_5_CODE_VISTA:
        #     kwargs['engine'] = os.environ['API_ENGINE']
        ...
        kwargs.update(
            {"max_tokens": 1024, "stop_sequences": None, "temperature": 0.2, "top_k": 0}
        )
        claude_output = self.client.messages.create(messages=messages, **kwargs)
        response = convert_claude_to_openai(claude_output)
