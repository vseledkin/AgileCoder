...
class ChatChain:
    ...
    def __init__(
        ...
        # init log
        self.start_time, self.log_filepath = self.get_logfile_path()
        ...
        self.compose_phase_module = importlib.import_module(
            "agilecoder.components.composed_phase"
        )
    ...
    def execute_step(self, phase_item: dict):
        ...
        if phase_type == "SimplePhase":
            ...
            if phase in self.phases:
                ...
                raise RuntimeError(
                    f"Phase '{phase}' is not yet implemented in components.phase"
                )
        # For ComposedPhase, we create instance here then conduct the "ComposedPhase.execute" method
            ...
            if not compose_phase_class:
                raise RuntimeError(
                    f"Phase '{phase}' is not yet implemented in components.compose_phase"
                )
    ...
    def post_processing(self):
        ...
        post_info += (
            f'Number of sprints {self.chat_env.env_dict.get("num-sprints", 1)}\n\n'
        )
